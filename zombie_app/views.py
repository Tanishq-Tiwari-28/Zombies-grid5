...
def download_pdf(request):
    ...
    molar_weight = request.GET.get('molar_weight')
    num_h_bonds = request.GET.get('num_h_bonds')
    boiling_pt = request.GET.get('boiling_pt')
    melting_pt = request.GET.get('melting_pt')
    log_p = request.GET.get('log_p')
    num_h_bond_acceptors = request.GET.get('num_h_bond_acceptors')
    num_h_bond_donors = request.GET.get('num_h_bond_donors')
    ...
    prop_bullet_points = (
    f"- Formula: {formula}\n"
    f"- Molar Weight: {molar_weight}\n"
    f"- Number of Hydrogen Bonds: {num_h_bonds}\n"
    f"- Boiling Point: {boiling_pt}\n"
    f"- Melting Point: {melting_pt}\n"
    f"- Log(P): {log_p}\n"
    f"- No. of HBond Donors: {num_h_bond_donors}\n"
    f"- No. of HBond Acceptors: {num_h_bond_acceptors}\n"
    )

    textLines = [
        f"Input Type: {input_type}",
        f"Input Data: {input_data}",
        "Molecular properties of Drugs:",prop_bullet_points, 
